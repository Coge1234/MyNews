...
import com.example.utils.SharedPreferencesTag;
...
public class UpdateReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        mSharedPreferencesHelper = mSharedPreferencesHelper
                .getInstance(NEWSApplication.getInstance());
        HashMap<String, Object> tempMap = NEWSApplication.getInstance()
                .getTempMap();
        UpdateInfoModel model = (UpdateInfoModel) tempMap
                .get(DeliverConsts.KEY_APP_UPDATE);
        try {
            UpdateInformation.localVersion = NEWSApplication
                    .getInstance().getPackageManager().getPackageInfo(
                            NEWSApplication.getInstance().getPackageName(), 0).versionCode;

            UpdateInformation.versionName = NEWSApplication
                    .getInstance().getPackageManager().getPackageInfo(
                            NEWSApplication.getInstance().getPackageName(), 0).versionName;

        } catch (Exception e) {
            e.printStackTrace();
        }
        UpdateInformation.appname = NEWSApplication.getInstance().getResources().getString(R.string.app_name);
    ...
    public void checkVersion(Context pContext) {
        if (UpdateInformation.localVersion < UpdateInformation.serverVersion) {
            // 需要进行更新
            mSharedPreferencesHelper.putIntValue(SharedPreferencesTag.IS_HAVE_NEW_VERSION, 1);
            update(pContext);
        } else {
            mSharedPreferencesHelper.putIntValue(SharedPreferencesTag.IS_HAVE_NEW_VERSION, 0);
